...
let areas: string[] = [];
let districtNames: Record<string, string> = {};
let inflationData: InflationData | null = null;
let allYearData: Record<string, Record<number, SectorData | null>> = {};
let propType = 'A';
let isReal = false;
...
function parseAreasFromUrl(): string[] {
...
function storeDistrictData(code: string, result: { name?: string; data?: Record<string, SectorData> } | null): void {
  allYearData[code] = {};
  if (result?.data) {
    YEARS.forEach((year) => {
      allYearData[code]![year] = result.data![String(year)] ?? null;
    });
    if (result.name) districtNames[code] = result.name;
  }
}
...
async function addArea(code: string): Promise<void> {
  areas.push(code);
  storeDistrictData(code, null);
  updateUrl();
  renderChips();
  if (window.posthog) window.posthog.capture('area_added_to_comparison', { postcode_district: code, total_areas: areas.length });
  try {
...
function trackAreas(codes: string[]): void {
  if (typeof window.sa_event !== 'function') return;
  const name = 'compare_area_' + codes.slice().sort().join('_').toLowerCase();
  window.sa_event(name);
  if (window.posthog) window.posthog.capture('comparison_viewed', { area_count: codes.length });
}
...
function renderChips(): void {
  ...
  areas.forEach(function (code, i) {
    ...
    chip.innerHTML =
      '<span class="compare-area-chip-dot" style="background:' +
      color +
      '"></span>' +
      '<span>' +
      code +
      ...
      (areas.length > 1
        ? '<button class="compare-area-chip-remove" aria-label="Remove ' + code + '">&times;</button>'
        : '');
...
function showAddAreaInput(wrap: HTMLElement, btn: HTMLElement): void {
  ...
  function cancelAdd(): void {
    if (input.parentNode) wrap.removeChild(input);
...
function getKnownCodes(): string[] {
  return Object.keys(districtNames);
}

function removeArea(code: string): void {
...
function computeAllAvg(sectorData: SectorData): PriceStats | null {
  ...
  return {
    ...
    median: medians.length ? Math.round(medians.reduce((a, b) => a + b, 0) / medians.length) : 0,
    count: totalCount,
  };
}

function getSeriesValue(code: string, year: number, metric: 'avg' | 'median' | 'count'): number | null {
  const sectorData = allYearData[code]?.[year];
  if (!sectorData) return null;

  let typeData = sectorData[propType];
  if (propType === 'A' && !typeData) {
    typeData = computeAllAvg(sectorData) ?? undefined;
  }
  if (!typeData) return null;

  if (metric === 'count') return typeData.count;

  let val = typeData[metric];
  if (val == null) return null;

  if (isReal && inflationData?.data) {
...
function buildDatasets(metric: 'avg' | 'median' | 'count') {
...
function priceTooltipLabel(ctx: any): string {
  const v = ctx.parsed.y;
  if (v === null || v === undefined) return ctx.dataset.label + ': No data';
  return ctx.dataset.label + ': \u00a3' + v.toLocaleString('en-GB');
}

function volumeTooltipLabel(ctx: any): string {
  const v = ctx.parsed.y;
  if (v === null || v === undefined) return ctx.dataset.label + ': No data';
  return ctx.dataset.label + ': ' + v.toLocaleString('en-GB') + ' sales';
}

function baseChartOptions(tooltipLabelFn: (ctx: any) => string, tickFn: (v: number) => string) {
  return {
    responsive: true,
...
function renderCharts(): void {
  const priceCtx = (document.getElementById('compare-price-chart') as HTMLCanvasElement).getContext('2d')!;
  const medianCtx = (document.getElementById('compare-median-chart') as HTMLCanvasElement).getContext('2d')!;
  const volumeCtx = (document.getElementById('compare-volume-chart') as HTMLCanvasElement).getContext('2d')!;
  ...
  volumeChart = new Chart(volumeCtx, {
    ...
    options: baseChartOptions(volumeTooltipLabel, function (v: number) {
...
function renderStats(): void {
  ...
  areas.forEach(function (code, i) {
    ...
    let latestStats: { avg: number } | null = null;
    ...
    if (latestYear) {
      const sectorData = allYearData[code]?.[latestYear];
      if (sectorData) {
        let td = sectorData[propType];
        if (propType === 'A' && !td) td = computeAllAvg(sectorData) ?? undefined;
        if (td) salesCount = td.count;
      }
    }

    const fmt = (p: number | null | undefined) => (p != null ? '\u00a3' + Math.round(p).toLocaleString('en-GB') : '\u2013');
    const fmtChange = (p: number | null) => (p != null ? (p >= 0 ? '+' + p + '%' : p + '%') : '\u2013');
    ...
    card.innerHTML =
      ...
      '<span class="compare-stat-card-dot" style="background:' +
      color +
      '"></span>' +
      '<span>' +
      code +
      ...
      '</div>' +
      '<div class="compare-stat-row">' +
      '<span class="compare-stat-label">Sales (' +
      (latestYear || '\u2013') +
      ')</span>' +
      '<span class="compare-stat-value">' +
      (salesCount != null ? salesCount.toLocaleString('en-GB') : '\u2013') +
      '</span>' +
      '</div>' +
      '<div class="compare-stat-row">' +
      '<span class="compare-stat-label">5-year change</span>' +
      '<span class="compare-stat-value" style="color:' +
      (fiveYearChange != null && fiveYearChange >= 0 ? '#27ae60' : '#c0392b') +
      '">' +
      fmtChange(fiveYearChange) +
      '</span>' +
      '</div>';
...
function setupControls(): void {
  const propSelect = document.getElementById('compare-prop-type') as HTMLSelectElement | null;
  if (propSelect) {
  ...
  document.querySelectorAll<HTMLElement>('.compare-mode-btn').forEach(function (btn) {
    btn.addEventListener('click', function () {
      isReal = btn.dataset['mode'] === 'real';
      document.querySelectorAll<HTMLElement>('.compare-mode-btn').forEach(function (b) {
